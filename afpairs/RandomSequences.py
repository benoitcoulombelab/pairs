...
def main(argv: list[str] = None):
    ...
    random_sequences(fasta=args.fasta, output=args.output, invalid_sequence=args.sequence)


def random_sequences(fasta: TextIO, output: TextIO, invalid_sequence: bool = False):
    """
    ...
    if invalid_sequence:
        input_sequences = [seq for seq in input_sequences if re.match(VALID_AMINO_ACID_REGEX, str(seq.seq))]
    ...
    for seq in input_sequences:
        ...
        output_sequences.append(SeqRecord(out_seq, id=name, description=f"MASS={mass}"))
...
def generate_sequence(length: int, aa_probabilities: dict[str, float]) -> Seq:
    ...
    """
    return Seq("".join(random.choices(list(aa_probabilities.keys()), list(aa_probabilities.values()), k=length)))
