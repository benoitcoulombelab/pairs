...
def main(argv: list[str] = None):
    ...
    parser.add_argument('-m', '--metric', choices=RANKING_METRICS, default=RANKING_METRICS[0],
                        help="Metric used to choose best model in ranking files  (default: %(default)s)")
    ...
    multi_interaction_score(input_dir=args.input, output_file=args.output, name=args.name,
                            metric=args.metric, progress=args.progress, recycled=args.recycled,
...
def multi_interaction_score(input_dir: str = "", output_file: TextIO = sys.stdout, name: str = r"([\w-]+)__([\w-]+)",
                            metric: str = RANKING_METRICS[0], progress: bool = False, recycled: bool = False,
    ...
    output_file.write("Bait\tTarget\tInterface score\tTM score\n")
    for ranking_file in (tqdm.tqdm(ranking_files) if progress else ranking_files):
        ...
        ranking = parse_rankings(ranking_file, metric, recycled)
        output_file.write(f"{ranking.interaction_score}\t{ranking.ptms}\n")
