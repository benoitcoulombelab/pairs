...
def main(argv: list[str] = None):
  ...
  delete_fasta(inputs=args.inputs, invalid_sequence=args.sequence,
               length=args.length, backup=args.backup,
               verbose=args.verbose)


def delete_fasta(inputs: list[str], invalid_sequence: bool = False,
    length: int = None, backup: str = None,
    verbose: bool = False):
  """
  ...
  for fasta in inputs:
    ...
    if invalid_sequence:
      any_invalid = [sequence for sequence in sequences if
                     re.match(VALID_AMINO_ACID_REGEX,
                              str(sequence.seq)) is None]
      if any_invalid:
        if verbose:
    ...
    if length is not None:
      ...
      if fasta_length > length:
        if verbose:
          print(
              f"Sequence length {fasta_length} above maximum {length} in FASTA file {fasta}")
        delete = True
    if delete:
      if backup:
