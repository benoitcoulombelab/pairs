...
def main(argv: list[str] = None):
  ...
  fasta_pairs(baits=args.baits, targets=args.targets, unique=args.unique,
              skip_identity=args.identity,
              output=args.output)


def fasta_pairs(baits: str, targets: str, unique: bool = False,
  ...
  for bait in baits:
    for target in targets:
      ...
      SeqIO.write([baits[bait], targets[target]],
                  os.path.join(output, f"{merge_id}.fasta"), "fasta")
