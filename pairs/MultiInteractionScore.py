...
class AlphafoldStatistics:
  def __init__(self, directory: str, ranked0_score: float,
      ranked0_confidence: float):
...
def main(argv: list[str] = None):
  parser = argparse.ArgumentParser(
      description="Compute protein-protein interaction score from multiple PDB files.")
  ...
  multi_interaction_score(input_files=args.inputs, output_file=args.output,
                          name=args.name,
                          stats=args.stats,
                          radius=args.radius, weight=args.weight,
                          count=args.count, progress=args.progress,
...
def multi_interaction_score(input_files: list[str],
    output_file: TextIO = sys.stdout, name: str = r"([\w-]+)__([\w-]+)",
    stats: bool = False,
    radius: float = 6, weight: bool = False, count: bool = False,
    progress: bool = False,
  ...
  for input_file in (tqdm.tqdm(input_files) if progress else input_files):
    ...
    if not re_match:
      raise AssertionError(
          f"Expression {name} cannot be found in filename {input_file}")
    ...
    if stats:
      a_s = alphafold_statistics(directory=os.path.dirname(input_file),
                                 radius=radius, weight=weight, count=count,
                                 first_chains=first_chains,
                                 second_chains=second_chains, partial=partial)
...
def alphafold_statistics(directory: str, radius: float = 6,
