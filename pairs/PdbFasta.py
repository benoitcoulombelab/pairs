...
def main(argv: list[str] = None):
  parser = argparse.ArgumentParser(
      description="Extracts protein sequences from PDB file and output them as FASTA.")
  parser.add_argument('pdb', nargs='?', type=argparse.FileType('r'),
                      default=sys.stdin,
                      help="PDB file")
...
def pdb_fasta(pdb: TextIO, output: TextIO, prefix: str = ""):
  ...
  for chain in structure[0]:
    sequence = seq1(
        "".join(residue.get_resname() for residue in chain.get_residues()))
    sequences.append(
        SeqRecord(Seq(sequence), id=f"{prefix}{chain.get_id()}",
                  description=""))
