...
def test_main(testdir, mock_testclass):
  ...
  output_file = Af2complexScore.multi_interaction_score.call_args.kwargs[
    "output_file"]
...
def test_main_parameters(testdir, mock_testclass):
  ...
  output_file = Af2complexScore.multi_interaction_score.call_args.kwargs[
    "output_file"]
  mapping_file = Af2complexScore.multi_interaction_score.call_args.kwargs[
    "mapping_file"]
  assert isinstance(output_file, TextIOWrapper)
...
def test_multi_interaction_score(testdir, mock_testclass):
  ...
  shutil.copy(Path(__file__).parent.joinpath("ranking_all_240603_519798.json"),
              ranking_file_2)
  ranking_1 = Af2complexScore.Ranking("model_5_multimer_v3_p1", 0.7772, 180,
                                      210, 0.7059, 89.2, 0.8952)
  ranking_2 = Af2complexScore.Ranking("model_4_multimer_v3_p1", 0.7601, 86, 93,
                                      0.783, 90.04, 0.8985)
  output = "output.txt"
  ...
  Af2complexScore.parse_rankings.assert_any_call(ranking_file_1, "interface",
                                                 False)
  Af2complexScore.parse_rankings.assert_any_call(ranking_file_2, "interface",
                                                 False)
...
def test_multi_interaction_score_parameters(testdir, mock_testclass):
  ...
  with open(output, "w") as output_out, open(mappings_file, "r") as mappings_in:
    Af2complexScore.multi_interaction_score("rankings", output_out,
                                            r"([\w-]+)___([\w-]+)",
                                            metrics, False, True, mappings_in,
                                            2, 3)
...
def test_multi_interaction_score_progress(testdir, mock_testclass):
  ...
  with open(output, "w") as output_out, patch("tqdm.tqdm",
                                              return_value=ranking_files) as mock_tqdm:
    Af2complexScore.multi_interaction_score(output_file=output_out,
                                            progress=True)
