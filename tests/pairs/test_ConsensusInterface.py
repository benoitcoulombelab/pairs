...
def test_main(testdir, mock_testclass):
  ...
  ConsensusInterface.consensus_interface.assert_called_once_with(
      residue_pair_files=ANY, output_file=ANY, name=r"([\w-]+)__([\w-]+)",
      consensus_ratio=0.5,
      baits_file=None, targets_file=None, alignment_format="clustal")
  residue_pair_in = ConsensusInterface.consensus_interface.call_args.kwargs[
    "residue_pair_files"]
  ...
  output_out = ConsensusInterface.consensus_interface.call_args.kwargs[
    "output_file"]
...
def test_main_parameters(testdir, mock_testclass):
  ...
  ConsensusInterface.consensus_interface.assert_called_once_with(
      residue_pair_files=ANY, output_file=ANY, name=name,
  ...
  baits_in = ConsensusInterface.consensus_interface.call_args.kwargs[
    "baits_file"]
  assert baits_in.name == baits_alignment
  assert baits_in.mode == "r"
  targets_in = ConsensusInterface.consensus_interface.call_args.kwargs[
    "targets_file"]
...
def test_main_long_parameters(testdir, mock_testclass):
  ...
  ConsensusInterface.consensus_interface = MagicMock()
  ConsensusInterface.main(
      ["--output", output_file, "--name", name, "--consensus",
       str(consensus_ratio),
       "--baits", baits_alignment, "--targets", targets_alignment, "--format",
       alignment_format,
...
def test_consensus_interface(testdir, mock_testclass):
  ...
  residue_pairs_1 = [
    ResiduePair(Residue("A", 7, "LEU"), Residue("B", 131, "GLU"), None)]
  residue_pairs_2 = [
    ResiduePair(Residue("A", 6, "LEU"), Residue("B", 131, "GLU"), None)]
  consensus_residue_pairs = [
    ConsensusResiduePair(8, 131).append(
        "HA_G4", "HEMA_I78A3",
        ResiduePair(Residue("A", 7, "LEU"), Residue("B", 131, "GLU"),
                    None)).append(
        "HEMA_I02A3", "HEMA_I78A3",
        ResiduePair(Residue("A", 6, "LEU"), Residue("B", 131, "GLU"), None)),
    ConsensusResiduePair(12, 275).append(
        "HA_G4", "HEMA_I78A3",
        ResiduePair(Residue("A", 11, "VAL"), Residue("B", 275, "ILE"),
                    "Hydrophobic")).append(
        "HEMA_I02A3", "HEMA_I03A0",
        ResiduePair(Residue("A", 10, "PHE"), Residue("B", 274, "LEU"), None)),
    ConsensusResiduePair(19, 421).append(
        "HA_G4", "HEMA_I78A3",
        ResiduePair(Residue("A", 19, "THR"), Residue("B", 420, "THR"),
                    "Charged")).append(
        "HEMA_I02A3", "HEMA_I03A0",
        ResiduePair(Residue("A", 18, "GLN"), Residue("B", 421, "ASP"),
                    "Hydrogen")).append(
        "HA_G4", "HEMA_I03A0",
        ResiduePair(Residue("A", 19, "THR"), Residue("B", 420, "THR"),
                    "Hydrogen"))]
  ConsensusInterface.parse_residue_pairs = MagicMock(
      side_effect=[residue_pairs_1, residue_pairs_2])
  ConsensusInterface.parse_alignment = MagicMock(
      side_effect=[baits_alignment, targets_alignment])
  ConsensusInterface.consensus_residue_pairs = MagicMock(
      return_value=consensus_residue_pairs)
  with open(residue_pairs_1_file, 'r') as residue_pairs_1_in, open(
      residue_pairs_2_file, 'r') as residue_pairs_2_in, \
      open(baits_alignment_file, 'r') as baits_in, open(targets_alignment_file,
                                                        'r') as targets_in, \
      open(output_file, 'w') as output_out:
    ConsensusInterface.consensus_interface(
        residue_pair_files=[residue_pairs_1_in, residue_pairs_2_in],
  ...
  ConsensusInterface.consensus_residue_pairs.assert_called_once_with(ANY,
                                                                     baits_alignment,
                                                                     targets_alignment)
...
def test_consensus_interface_no_alignment(testdir, mock_testclass):
  ...
  with open(residue_pairs_1_file, 'r') as residue_pairs_1_in, open(
    ...
    ConsensusInterface.parse_residue_pairs.assert_any_call(residue_pairs_1_in)
    ConsensusInterface.parse_residue_pairs.assert_any_call(residue_pairs_2_in)
  ConsensusInterface.consensus_residue_pairs.assert_called_once_with(ANY, None,
                                                                     None)
  residue_pairs = ConsensusInterface.consensus_residue_pairs.call_args.args[0]
  assert len(residue_pairs) == 2
  assert ("HA_G4", "HEMA_I78A3") in residue_pairs
...
def test_consensus_interface_missing_bait_in_alignment(testdir, mock_testclass):
  ...
  with open(residue_pairs_1_file, 'r') as residue_pairs_1_in, open(
      ...
      open(baits_alignment_file, 'r') as baits_in, open(targets_alignment_file,
                                                        'r') as targets_in, \
      open(output_file, 'w') as output_out, pytest.raises(
      AssertionError) as e_info:
    ConsensusInterface.consensus_interface(
        residue_pair_files=[residue_pairs_1_in, residue_pairs_2_in],
        output_file=output_out,
        baits_file=baits_in, targets_file=targets_in)
...
def test_consensus_interface_missing_target_in_alignment(testdir,
    mock_testclass):
  residue_pairs_1_file = "HA_G4__HEMA_I78A3.txt"
  residue_pairs_2_file = "HEMA_I02A3__HEMA_I03A0.txt"
  open(residue_pairs_1_file, 'w').close()
...
def test_consensus_residue_pairs(testdir, mock_testclass):
  ...
  residue_pairs[(baits[0], target)].append(
      ResiduePair(Residue("A", 5, "PHE"), Residue("B", 280, "TYR"),
                  "Hydrophobic"))
  residue_pairs[(baits[0], target)].append(
      ResiduePair(Residue("A", 534, "TYR"), Residue("B", 455, "LYS"),
                  "Hydrogen"))
  ...
  assert consensus_residue_pair.residue_pairs[(baits[0], target)] == \
         residue_pairs[(baits[0], target)][0]
  assert (baits[1], target) in consensus_residue_pair.residue_pairs
  assert consensus_residue_pair.residue_pairs[(baits[1], target)] == \
         residue_pairs[(baits[1], target)][0]
  assert (baits[2], target) in consensus_residue_pair.residue_pairs
  assert consensus_residue_pair.residue_pairs[(baits[2], target)] == \
         residue_pairs[(baits[2], target)][0]
  ...
  assert (baits[0], target) in consensus_residue_pair.residue_pairs
  assert consensus_residue_pair.residue_pairs[(baits[0], target)] == \
         residue_pairs[(baits[0], target)][1]
  ...
  assert (baits[0], target) in consensus_residue_pair.residue_pairs
  assert consensus_residue_pair.residue_pairs[(baits[0], target)] == \
         residue_pairs[(baits[0], target)][2]
  assert (baits[1], target) in consensus_residue_pair.residue_pairs
  assert consensus_residue_pair.residue_pairs[(baits[1], target)] == \
         residue_pairs[(baits[1], target)][1]
  ...
  assert (baits[2], target) in consensus_residue_pair.residue_pairs
  assert consensus_residue_pair.residue_pairs[(baits[2], target)] == \
         residue_pairs[(baits[2], target)][1]
...
def test_consensus_residue_pairs_bait_align(testdir, mock_testclass):
  ...
  residue_pairs[(baits[0], target)].append(
      ResiduePair(Residue("A", 5, "PHE"), Residue("B", 280, "TYR"),
                  "Hydrophobic"))
  residue_pairs[(baits[0], target)].append(
      ResiduePair(Residue("A", 530, "TYR"), Residue("B", 455, "LYS"),
                  "Hydrogen"))
  residue_pairs[(baits[1], target)].append(
      ResiduePair(Residue("A", 1, "MET"), Residue("B", 130, "GLU"), None))
  residue_pairs[(baits[1], target)].append(
      ResiduePair(Residue("A", 526, "TYR"), Residue("B", 455, "LYS"),
                  "Hydrogen"))
  residue_pairs[(baits[2], target)].append(
...
def test_consensus_residue_pairs_target_align(testdir, mock_testclass):
  ...
  residue_pairs[(bait, targets[0])].append(
      ResiduePair(Residue("B", 280, "TYR"), Residue("A", 5, "PHE"),
                  "Hydrophobic"))
  residue_pairs[(bait, targets[0])].append(
      ResiduePair(Residue("B", 455, "LYS"), Residue("A", 530, "TYR"),
                  "Hydrogen"))
  residue_pairs[(bait, targets[1])].append(
      ResiduePair(Residue("B", 130, "GLU"), Residue("A", 1, "MET"), None))
  residue_pairs[(bait, targets[1])].append(
      ResiduePair(Residue("B", 455, "LYS"), Residue("A", 526, "TYR"),
                  "Hydrogen"))
  ...
  with open(alignment_file, "r") as alignment_in:
    alignment = ConsensusInterface.parse_alignment(alignment_in, "clustal")
  consensus_residue_pairs = ConsensusInterface.consensus_residue_pairs(
      residue_pairs, targets=alignment)
  assert len(consensus_residue_pairs) == 4
  consensus_residue_pair = consensus_residue_pairs[0]
  ...
  assert consensus_residue_pair.residue_pairs[(bait, targets[0])] == \
         residue_pairs[(bait, targets[0])][0]
  assert (bait, targets[1]) in consensus_residue_pair.residue_pairs
  assert consensus_residue_pair.residue_pairs[(bait, targets[1])] == \
         residue_pairs[(bait, targets[1])][0]
  assert (bait, targets[2]) in consensus_residue_pair.residue_pairs
  assert consensus_residue_pair.residue_pairs[(bait, targets[2])] == \
         residue_pairs[(bait, targets[2])][0]
  consensus_residue_pair = consensus_residue_pairs[1]
  ...
  assert (bait, targets[0]) in consensus_residue_pair.residue_pairs
  assert consensus_residue_pair.residue_pairs[(bait, targets[0])] == \
         residue_pairs[(bait, targets[0])][1]
  consensus_residue_pair = consensus_residue_pairs[2]
  ...
  assert (bait, targets[0]) in consensus_residue_pair.residue_pairs
  assert consensus_residue_pair.residue_pairs[(bait, targets[0])] == \
         residue_pairs[(bait, targets[0])][2]
  assert (bait, targets[1]) in consensus_residue_pair.residue_pairs
  assert consensus_residue_pair.residue_pairs[(bait, targets[1])] == \
         residue_pairs[(bait, targets[1])][1]
  consensus_residue_pair = consensus_residue_pairs[3]
  ...
  assert (bait, targets[2]) in consensus_residue_pair.residue_pairs
  assert consensus_residue_pair.residue_pairs[(bait, targets[2])] == \
         residue_pairs[(bait, targets[2])][1]
...
def test_consensus_residue_pairs_bait_align(testdir, mock_testclass):
  ...
  residue_pairs[(baits[1], targets[0])].append(
      ResiduePair(Residue("A", 532, "TYR"), Residue("B", 455, "LYS"),
                  "Hydrogen"))
  ...
  residue_pairs[(baits[1], targets[1])].append(
      ResiduePair(Residue("A", 532, "LYS"), Residue("B", 461, "GLU"),
                  "Hydrogen"))
  alignment_file = Path(__file__).parent.joinpath("clustal_alignment.txt")
  with open(alignment_file, "r") as alignment_in:
    alignment = ConsensusInterface.parse_alignment(alignment_in, "clustal")
  ...
  assert consensus_residue_pair.residue_pairs[(baits[0], targets[0])] == \
         residue_pairs[(baits[0], targets[0])][0]
  assert (baits[1], targets[0]) in consensus_residue_pair.residue_pairs
  assert consensus_residue_pair.residue_pairs[(baits[1], targets[0])] == \
         residue_pairs[(baits[1], targets[0])][0]
  assert (baits[0], targets[1]) in consensus_residue_pair.residue_pairs
  assert consensus_residue_pair.residue_pairs[(baits[0], targets[1])] == \
         residue_pairs[(baits[0], targets[1])][0]
  consensus_residue_pair = consensus_residue_pairs[1]
  assert consensus_residue_pair.bait_residue_index == 5
  ...
  assert (baits[0], targets[0]) in consensus_residue_pair.residue_pairs
  assert consensus_residue_pair.residue_pairs[(baits[0], targets[0])] == \
         residue_pairs[(baits[0], targets[0])][1]
  consensus_residue_pair = consensus_residue_pairs[2]
  assert consensus_residue_pair.bait_residue_index == 534
  ...
  assert (baits[0], targets[0]) in consensus_residue_pair.residue_pairs
  assert consensus_residue_pair.residue_pairs[(baits[0], targets[0])] == \
         residue_pairs[(baits[0], targets[0])][2]
  assert (baits[1], targets[0]) in consensus_residue_pair.residue_pairs
  assert consensus_residue_pair.residue_pairs[(baits[1], targets[0])] == \
         residue_pairs[(baits[1], targets[0])][1]
  assert (baits[1], targets[1]) in consensus_residue_pair.residue_pairs
  assert consensus_residue_pair.residue_pairs[(baits[1], targets[1])] == \
         residue_pairs[(baits[1], targets[1])][0]
  consensus_residue_pair = consensus_residue_pairs[3]
  ...
  assert (baits[0], targets[1]) in consensus_residue_pair.residue_pairs
  assert consensus_residue_pair.residue_pairs[(baits[0], targets[1])] == \
         residue_pairs[(baits[0], targets[1])][1]
