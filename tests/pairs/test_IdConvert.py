...
def test_main(testdir, mock_testclass):
  ...
  assert isinstance(IdConvert.id_convert.call_args.kwargs["output_file"],
                    TextIOWrapper)
  assert IdConvert.id_convert.call_args.kwargs["output_file"].mode in ["r+",
                                                                       "w"]
  assert IdConvert.id_convert.call_args.kwargs[
           "mapping_file"].name == mapping_file
...
def test_main_parameters(testdir, mock_testclass):
  ...
  IdConvert.id_convert = MagicMock()
  IdConvert.main(
      ["-i", "2", "-m", mapping_file, "-s", "3", "-c", "4", input_file,
       output_file])
  IdConvert.id_convert.assert_called_once_with(
...
def test_id_convert(testdir, mock_testclass):
  ...
  with open(input_file, 'r') as input_in, open(mapping_file, 'r') as mapping_in, \
    ...
    IdConvert.id_convert(input_file=input_in, output_file=output_out,
