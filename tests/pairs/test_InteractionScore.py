...
def test_main_long_parameters(testdir, mock_testclass):
  ...
  smokesignal.on = MagicMock()
  InteractionScore.main(
      ["--first", "A,B", "--second", "C,D", "--radius", "8", "--weight",
       "--count",
       "--residues", residue_pairs_file, "--atoms", atom_pairs_file, "--output",
       output_file,
       "--partial", input_file])
  InteractionScore.interaction_score.assert_called_once_with(
      structure_file=ANY, radius=8, weight=True, count=True,
...
def test_interaction_score(testdir, mock_testclass):
  ...
  with open(pdb, "r") as pdb_in:
    score = InteractionScore.interaction_score(structure_file=pdb_in)
...
def test_interaction_score_write_residues(testdir, mock_testclass):
  ...
  with open(pdb, "r") as pdb_in, open(residue_pairs_file,
                                      'w') as residue_pairs_out:
    InteractionScore.interaction_score(structure_file=pdb_in,
...
def test_interaction_score_write_atoms(testdir, mock_testclass):
  ...
  with open(pdb, "r") as pdb_in, open(atom_pairs_file, 'w') as atom_pairs_out:
    InteractionScore.interaction_score(structure_file=pdb_in,
                                       atoms=atom_pairs_out)
...
def test_interaction_score_unused_chain_partial(testdir, mock_testclass,
    capsys):
  smokesignal.on(InteractionScore.MISSING_CHAIN_EVENT,
                 InteractionScore.warn_missing_chain, max_calls=1)
  pdb = Path(__file__).parent.joinpath("FAB_5_3__HVM13_MOUSE_ranked_0.pdb")
  with open(pdb, "r") as pdb_in:
    InteractionScore.interaction_score(structure_file=pdb_in, partial=True)
  out, err = capsys.readouterr()
  sys.stderr.write(err)
  assert "Chain C present in PDB but not used for scoring\n" not in err


def test_interaction_score_unused_chain_multiple_pdb(testdir, mock_testclass,
    capsys):
  smokesignal.on(InteractionScore.MISSING_CHAIN_EVENT,
                 InteractionScore.warn_missing_chain, max_calls=1)
  pdb = Path(__file__).parent.joinpath("FAB_5_3__HVM13_MOUSE_ranked_0.pdb")
...
def test_minimal_distance(mock_testclass):
  ...
  InteractionScore.potential_interactor_atoms = MagicMock(
      side_effect=[atoms_a_pro, atoms_b_asp])
...
def test_search_interactions_residue(mock_testclass):
  ...
  atoms_a = [chain_a[13]["CB"], chain_a[13]["SG"],
             ...
             chain_a[15]["CB"], chain_a[15]["CG"], chain_a[15]["CD1"],
             chain_a[15]["CD2"]]
  chain_b = structure[0]["B"]
  atoms_b = [chain_b[1147]["CB"], chain_b[1147]["OG"],
             chain_b[1148]["CB"], chain_b[1148]["CG"], chain_b[1148]["CD1"],
             chain_b[1148]["CD2"],
             chain_b[1149]["CB"], chain_b[1149]["CG1"], chain_b[1149]["CG2"]]
  neighbor_search = NeighborSearch(atoms_a + atoms_b)
  residue_pairs = InteractionScore.search_interactions(
      neighbor_search=neighbor_search, radius=6.0, level='R')
...
def test_search_interactions_atom(mock_testclass):
  ...
  atoms_b = [chain_b[1147]["CB"], chain_b[1147]["OG"],
             ...
             chain_b[1149]["CB"], chain_b[1149]["CG1"], chain_b[1149]["CG2"]]
  neighbor_search = NeighborSearch(atoms_a + atoms_b)
  atoms_pairs = InteractionScore.search_interactions(
      neighbor_search=neighbor_search, radius=6.0, level='A')
...
def test_write_residues(testdir, mock_testclass):
  ...
  with open(output_file, 'w') as output_out:
    InteractionScore.write_residues(residue_pairs=residue_pairs,
                                    output_file=output_out)
  InteractionScore.minimal_distance.assert_any_call(structure[0]["A"][1],
                                                    structure[0]["B"][1])
  InteractionScore.minimal_distance.assert_any_call(structure[0]["A"][2],
                                                    structure[0]["B"][2])
  InteractionScore.minimal_distance.assert_any_call(structure[0]["A"][3],
                                                    structure[0]["B"][3])
  InteractionScore.is_charged_bond.assert_any_call(structure[0]["A"][1],
                                                   structure[0]["B"][1])
  InteractionScore.is_charged_bond.assert_any_call(structure[0]["A"][2],
                                                   structure[0]["B"][2])
  InteractionScore.is_charged_bond.assert_any_call(structure[0]["A"][3],
                                                   structure[0]["B"][3])
  InteractionScore.is_hydrophobic_bond.assert_any_call(structure[0]["A"][2],
                                                       structure[0]["B"][2])
  InteractionScore.is_hydrophobic_bond.assert_any_call(structure[0]["A"][3],
                                                       structure[0]["B"][3])
  InteractionScore.is_hydrogen_bond.assert_any_call(structure[0]["A"][3],
                                                    structure[0]["B"][3])
  os.path.isfile(output_file)
  with open(output_file, 'r') as output_in:
...
def test_write_atoms(testdir, mock_testclass):
  ...
  InteractionScore.minimal_distance.assert_any_call(structure[0]["A"][1]["C"],
                                                    structure[0]["B"][1]["N"])
  InteractionScore.minimal_distance.assert_any_call(structure[0]["A"][2]["CA"],
                                                    structure[0]["B"][2]["O"])
  InteractionScore.minimal_distance.assert_any_call(structure[0]["A"][3]["CA"],
                                                    structure[0]["B"][3]["CG"])
  InteractionScore.is_charged_bond.assert_any_call(structure[0]["A"][1],
                                                   structure[0]["B"][1])
  InteractionScore.is_charged_bond.assert_any_call(structure[0]["A"][2],
