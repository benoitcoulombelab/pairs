...
def create_af2complex_files(af2complex_output, run_ids):
    ...
    if len(run_ids) == 1:
        ...
        create_files.extend([f"model_{i}_multimer_v3_p1_{run_ids[0]}.pdb" for i in range(1, 6)])
        create_files.extend([f"relaxed_model_{i}_multimer_v3_p1_{run_ids[0]}.pdb" for i in range(1, 6)])
        create_files.extend([f"model_{i}_multimer_v3_p1_{run_ids[0]}.pkl" for i in range(1, 6)])
        create_files.extend([f"recycled/model_{i}_multimer_v3_p1_{run_ids[0]}_recycled_0{j}.pdb"
                             for i in range(1, 6) for j in range(0, 5)])
    else:
        create_files = ["relax_metrics.json"]
        create_files.extend([f"ranking_model_{i}_multimer_v3_p1_{run_ids[i - 1]}.json" for i in range(1, 6)])
        create_files.extend([f"model_{i}_multimer_v3_p1_{run_ids[i - 1]}.pdb" for i in range(1, 6)])
        create_files.extend([f"relaxed_model_{i}_multimer_v3_p1_{run_ids[i - 1]}.pdb" for i in range(1, 6)])
        create_files.extend([f"model_{i}_multimer_v3_p1_{run_ids[i - 1]}.pkl" for i in range(1, 6)])
        create_files.extend([f"recycled/model_{i}_multimer_v3_p1_{run_ids[i - 1]}_recycled_0{j}.pdb"
                             for i in range(1, 6) for j in range(0, 5)])
    create_files.extend([f"checkpoint/model_{i}_multimer_v3_p1_False.pkl" for i in range(1, 6)])
    create_files.extend([f"checkpoint/model_{i}_multimer_v3_p1_checkpoint.pkl" for i in range(1, 6)])
...
def test_main_long_parameters(testdir, mock_testclass):
    ...
    ListFiles.list_files = MagicMock()
    ListFiles.main(
        ["--output", output_file, "--progress", "--metric", metric, "--all-pdb", "--no-pkl", "--all-pkl", input_dir])
    ListFiles.list_files.assert_called_once_with(
        input_dir=input_dir, output_file=ANY, progress=True, metric=metric,
...
def test_list_files_alphafold(testdir, mock_testclass):
    ...
    shutil.copy(Path(__file__).parent.joinpath("ranking_debug.json"), "RPB1_RPB2/ranking_debug.json")
    shutil.copy(Path(__file__).parent.joinpath("ranking_debug_2.json"), "RPB3_RPB4/ranking_debug.json")
    shutil.copy(Path(__file__).parent.joinpath("ranking_debug.json"), "RPB5_RPB6/ranking_debug.json")
    shutil.copy(Path(__file__).parent.joinpath("ranking_debug_2.json"), "RPB7_RPB8/ranking_debug.json")
    output_file = "output.txt"
...
def test_list_files_af2complex(testdir, mock_testclass):
    ...
    run_ids = [["240525_625637"], ["240624_450814", "240624_525588", "240624_260995", "240624_214651", "240624_536961"],
               ["240603_519798"], ["240525_625637"]]
    [testdir.mkdir(af2complex_outputs[i]) for i in range(0, 4)]
    [create_af2complex_files(af2complex_outputs[i], run_ids[i]) for i in range(0, 4)]
    shutil.copy(Path(__file__).parent.joinpath("ranking_all_240525_625637.json"),
                f"RPB1_RPB2/ranking_all_240525_625637.json")
    shutil.copy(Path(__file__).parent.joinpath("ranking_model_1_multimer_v3_p1_240624_450814.json"),
                f"RPB3_RPB4/ranking_model_1_multimer_v3_p1_240624_450814.json")
    shutil.copy(Path(__file__).parent.joinpath("ranking_model_2_multimer_v3_p1_240624_525588.json"),
                f"RPB3_RPB4/ranking_model_2_multimer_v3_p1_240624_525588.json")
    shutil.copy(Path(__file__).parent.joinpath("ranking_model_3_multimer_v3_p1_240624_260995.json"),
                f"RPB3_RPB4/ranking_model_3_multimer_v3_p1_240624_260995.json")
    shutil.copy(Path(__file__).parent.joinpath("ranking_model_4_multimer_v3_p1_240624_214651.json"),
                f"RPB3_RPB4/ranking_model_4_multimer_v3_p1_240624_214651.json")
    shutil.copy(Path(__file__).parent.joinpath("ranking_model_5_multimer_v3_p1_240624_536961.json"),
...
def test_list_files_af2complex_metric(testdir, mock_testclass):
    ...
    [create_af2complex_files(af2complex_outputs[i], run_ids[i]) for i in range(0, 2)]
    shutil.copy(Path(__file__).parent.joinpath("ranking_all_240525_625637.json"),
                f"RPB1_RPB2/ranking_all_240525_625637.json")
    shutil.copy(Path(__file__).parent.joinpath("ranking_model_1_multimer_v3_p1_240624_450814.json"),
                f"RPB3_RPB4/ranking_model_1_multimer_v3_p1_240624_450814.json")
...
def test_list_files_progress(testdir, mock_testclass):
    ...
    shutil.copy(Path(__file__).parent.joinpath("ranking_debug_2.json"), "RPB3_RPB4/ranking_debug.json")
    output_file = "output.txt"
    with open(output_file, 'w') as output_out, patch("tqdm.tqdm", return_value=alphafold_outputs) as mock_tqdm:
...
def test_get_best_model_af2complex_per_model_ranking(testdir, mock_testclass):
    ranking_files = [Path(__file__).parent.joinpath(f"ranking_model_1_multimer_v3_p1_240624_450814.json"),
                     Path(__file__).parent.joinpath(f"ranking_model_2_multimer_v3_p1_240624_525588.json"),
                     Path(__file__).parent.joinpath(f"ranking_model_3_multimer_v3_p1_240624_260995.json"),
                     Path(__file__).parent.joinpath(f"ranking_model_4_multimer_v3_p1_240624_214651.json"),
