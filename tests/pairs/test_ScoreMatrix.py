...
def test_main_parameters(testdir, mock_testclass):
  ...
  ScoreMatrix.score_matrix = MagicMock()
  ScoreMatrix.main(
      ["-z", "-u", "-o", output_file, scores_file_1, scores_file_2])
  ScoreMatrix.score_matrix.assert_called_once_with(
...
def test_main_long_parameters(testdir, mock_testclass):
  ...
  ScoreMatrix.score_matrix = MagicMock()
  ScoreMatrix.main(
      ["--zscore", "--unique", "--output", output_file, scores_file_1,
       scores_file_2])
  ScoreMatrix.score_matrix.assert_called_once_with(
      score_files=ANY, output_file=ANY, zscore=True, unique=True)
  score_files = ScoreMatrix.score_matrix.call_args.kwargs["score_files"]
...
def test_score_matrix(testdir, mock_testclass):
  ...
  ScoreMatrix.parse_scores = MagicMock(
      side_effect=[interactions_1, interactions_2])
  random_data = numpy.random.rand(5, 3)
  matrix = pandas.DataFrame(random_data, columns=["POLR2A", "POLR2B", "POLR2C"],
                            index=["POLR2A", "POLR2B", "POLR2C", "POLR2D",
                                   "POLR2E"])
  ScoreMatrix.interaction_matrix = MagicMock(return_value=matrix)
  print(random_data[0])
  with open(scores_file_1, 'r') as scores_in_1, open(scores_file_2,
                                                     'r') as scores_in_2, \
  ...
  ScoreMatrix.interaction_matrix.assert_called_once_with(
      interactions_1 + interactions_2, zscore=False, unique=False)
  with open(output_file, 'r') as output_in:
    assert output_in.readline() == "Target\tPOLR2A\tPOLR2B\tPOLR2C\n"
    assert output_in.readline() == "POLR2A\t" + "\t".join(
        [str(d) for d in random_data[0]]) + "\n"
    assert output_in.readline() == "POLR2B\t" + "\t".join(
        [str(d) for d in random_data[1]]) + "\n"
    assert output_in.readline() == "POLR2C\t" + "\t".join(
        [str(d) for d in random_data[2]]) + "\n"
    assert output_in.readline() == "POLR2D\t" + "\t".join(
        [str(d) for d in random_data[3]]) + "\n"
    assert output_in.readline() == "POLR2E\t" + "\t".join(
        [str(d) for d in random_data[4]]) + "\n"
...
def test_score_matrix_unique_zscore(testdir, mock_testclass):
  ...
  with open(scores_file_1, 'r') as scores_in_1, open(scores_file_2,
      ...
      open(output_file, 'w') as output_out:
    ScoreMatrix.score_matrix([scores_in_1, scores_in_2], output_file=output_out,
                             zscore=True, unique=True)
    ScoreMatrix.parse_scores.assert_any_call(scores_in_1)
    ScoreMatrix.parse_scores.assert_any_call(scores_in_2)
  ScoreMatrix.interaction_matrix.assert_called_once_with(
      interactions_1 + interactions_2, zscore=True, unique=True)
  with open(output_file, 'r') as output_in:
    assert output_in.readline() == "Target\tPOLR2A\tPOLR2B\tPOLR2C\n"
    assert output_in.readline() == "POLR2A\t" + "\t".join(
        [str(d) for d in random_data[0]]) + "\n"
...
def test_interaction_matrix_unique(mock_testclass):
  ...
  random_matrix = pandas.DataFrame(random_data,
                                   columns=["POLR2A", "POLR2B", "POLR2C",
                                            "POLR2D", "POLR2E"],
                                   index=["POLR2A", "POLR2B", "POLR2C",
                                          "POLR2D", "POLR2E"])
...
def test_interaction_matrix_zscore_unique(mock_testclass):
  ...
  random_matrix_3 = pandas.DataFrame(numpy.random.rand(5, 5),
                                     ...
                                     index=["POLR2A", "POLR2B", "POLR2C",
                                            "POLR2D", "POLR2E"])
  ScoreMatrix.unique_matrix = MagicMock(
      side_effect=[random_matrix_1, random_matrix_2])
...
def test_zscore(mock_testclass):
  ...
  for i in matrix.index:
    for j in matrix.columns:
      assert (random_matrix.at[i, j] - means[j]) / standard_deviations[j] == \
             matrix.at[i, j]
...
def test_unique_matrix(mock_testclass):
  ...
  for i in matrix.index:
    for j in matrix.columns:
      assert max(random_matrix.at[i, j], random_matrix.at[j, i]) == matrix.at[
        i, j]
...
def test_unique_matrix_nan(mock_testclass):
  ...
  for i in matrix.index:
    for j in matrix.columns:
      if math.isnan(random_matrix.at[i, j]) and math.isnan(
          random_matrix.at[j, i]):
